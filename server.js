...
app.set('ipaddr', process.env.OPENSHIFT_NODEJS_IP || "192.168.60.213");
//app.set('ipaddr', process.env.OPENSHIFT_NODEJS_IP || "192.168.1.102");
...
io.sockets.on("connection", function (socket) {
	...
	socket.on("private", function(id,msg) {
		var whisperTo = id;
		var whisperMsg = msg;
		socket.emit("whisper", whisperTo, {name: "You"}, whisperMsg);
		io.sockets.socket(whisperTo).emit("whisper", socket.id, people[socket.id], whisperMsg);
	});
	...
	socket.on("disconnect", function() {
		if (typeof people[socket.id] !== "undefined") { //this handles the refresh of the name screen
			...
			io.sockets.emit("update-people-disconnected", people[socket.id].name, socket.id);
